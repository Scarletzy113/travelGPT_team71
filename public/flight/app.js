  ...
  results.forEach(({ itineraries, price }) => {
    ...
    searchResults.insertAdjacentHTML(
      ...
      `<li class="flex-column flex-sm-row list-group-item d-flex justify-content-between align-items-sm-center">
        ...
        <button class="btn btn-primary save-flight-btn" data-flight='${JSON.stringify(itineraries)}'>Save</button>
      </li>`
    );

    
  });
  const saveButtons = document.querySelectorAll('.save-flight-btn');
  saveButtons.forEach((button) => {
    button.addEventListener('click', handleSaveFlight);
  });
};

const handleSaveFlight = async (event) => {
